...
- CI workflow with structural validation, security scan, mock tool execution, YAML lint, registry + JSON-schema validation, spec-version enforcement, and link checking
- In-repo validators (no external CLI dependency)
- `grok_install_stub` — stub runtime for CI
- SVG poster cards for every template, generated by `scripts/gen_poster.py`
- JSON Schema ([schemas/featured-agents.schema.json](schemas/featured-agents.schema.json)) for the registry
- Tag-triggered release workflow that publishes GitHub Releases from `CHANGELOG.md`
- Optional `visuals:` block (grok-install `v2.14`) on 5 hero templates
...
CI runs `yamllint`, `validate_registry`, an `ajv-cli` JSON-schema check,
a `spec-version` gate (templates must declare `grok-install/v2.12`,
`v2.13`, or `v2.14`), a `lychee` link check on template READMEs, and
— per template — `validate_template`, `scan_template`, and
`mock_run_template`. See
[.github/workflows/validate-templates.yml](.github/workflows/validate-templates.yml).
Tagged `v*` pushes also trigger
[.github/workflows/release.yml](.github/workflows/release.yml), which
publishes a GitHub Release with notes pulled from
